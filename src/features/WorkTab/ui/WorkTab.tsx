...
import { WorkPlaces, WorkPlaceWithConditions } from "../../../entities/Workplaces/model/types";
import { StudiedSpecialStats, StudiedStats, StudiedStatWithСondition } from "../../../entities/StudiedStats";
...
export const WorkTab = observer(() => {
  ...
  //const onClick = (key: string, text: string, money: number) => {
   const onClick = (key: string ) => {
    const imgPath = `${process.env.PUBLIC_URL}/girls/${
      slaveStore.slave.id
  ...
  return (
    <Flex bg="#dddcf7"minW="100%" flexDir={"column"} minH="100%" justifyContent={"space-between"}>
      ...
      <Text p={4}>
        ...
        сюда — это несомненный профессионал, как минимум умеющий ходить и писать
        своё имя. Старенькие корпоративные компьютеры, купленные на Авито,
        составляют компанию кипам документов на офисных столах. У стены гордо
        стоит кофеварка, заправляющая работников так нужной им энергией, а
        широкие окна повтоянно напоминают им, что солнце ещё высоко. Здесь твой
        любимый работник может принести весомый вклад в общество и
